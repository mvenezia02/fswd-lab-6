...
app.post('/users/register', function(request, response){
      if(request.body.name && request.body.password === request.body.confirmPassword){
          console.log('user name is '+request.body.name);
          // check for unique user name
          models.User.findOne({where: {name: request.body.name}, attributes:['name']})
            .then(function(user){
              console.log('user is ' + user);
              if(user){response.send('name already in use');}
              if(!user){
                models.User.create({ name: request.body.name, password: request.body.password})
                  .then(function(newUser){
                    response.send('new user ' + newUser.name + " created");
                  });
